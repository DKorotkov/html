               ...
               if (rule.type == "rule") {
                  ...
                  Object.keys(declarations).forEach(function (i) {
                     ...
                     if (declaration.property.indexOf("background") + 1 && declaration.value.indexOf("url") + 1 && !(declaration.value.indexOf(".webp") + 1) && !(declaration.value.indexOf(".svg") + 1)) {
                        ...
                        var path = newValue.slice(newValue.indexOf("url"), newValue.lastIndexOf(")"));
                        
                        if (path.indexOf("ImgToResp") > 0) {
                           var fileName = path.slice(path.lastIndexOf("/") + 1, path.lastIndexOf("-"));
                           var pathName = path.slice(path.lastIndexOf("/") + 1, path.lastIndexOf("-"));
                           var newPath = path.replace(fileName, pathName + "/" + fileName);
                        }
                        else {
